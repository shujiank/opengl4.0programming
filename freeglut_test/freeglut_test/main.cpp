...
void CreateCube(void)
{
	...
	ShaderIds[3] = LoadShader("SimpleShader.tessControl.glsl", GL_TESS_CONTROL_SHADER);
	ShaderIds[4] = LoadShader("SimpleShader.tessEval.glsl", GL_TESS_EVALUATION_SHADER);
	ShaderIds[5] = LoadShader("SimpleShader.geometry.glsl", GL_GEOMETRY_SHADER);
	...
	ExitOnGLError("ERROR: Could not get the shader uniform locations, 3");
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 4 * sizeof(Verts[0]), (GLvoid*)0);
...
void DrawCube(void)
{
	...
	//glGetUniformuiv(ShaderIds[0], size_location, &sizep);
